 ...
 var tuple = function(array) {
  ...
  cls('Screen', [], {
    init_screen: def([640, 480], function(self, width, height) {
      ...
      document.addEventListener('mousedown', function(e) {
        self._updateMouse(self, e);
        self._buttons[e.button] = true;

        self._events.push({
          type: 'MOUSEBUTTONDOWN',
          // TODO translate events
          pos: tuple(self._toWorld(self, e.clientX, e.clientY)),
          button: e.button + 1,
        });
        e.preventDefault();
        self.root.focus();
      ...
      document.addEventListener('touchstart', function(e) {
        ...
        for (var i = 0; i < e.changedTouches.length; i++) {
          self._updateMouse(self, e.changedTouches[i]);
          self._events.push({
            type: 'MOUSEBUTTONDOWN',
      ...
      document.addEventListener('touchmove', function(e) {
        self._updateMouse(self, e.changedTouches[0]);
      });
    ...
    _updateMouse: function(self, e) {
      var pos = self._toWorld(self, e.clientX, e.clientY);
      self.mouseX = pos[0];
      self.mouseY = pos[1];
    },
  ...
  var Obj = cls('Object', [], {
    __init__: def([0, 0, 0, 0], function(self, screen, x, y, surface, a, x_offset, y_offset, static) {
      ...
      call(self.screen.add_object, self.screen, self);
  ...
  var Sound = cls('Sound', [], {
    __init__: def([], function(self, audio) {
      self.audio = audio;
    }),
  ...
  mod.load_sound = def([], function(file) {
    return promise(function(resolve) {
      ...
      audio.addEventListener('canplaythrough', function(e) {
        resolve(Sk.misceval.callsim(Sound, audio));
      });
      audio.addEventListener('error', function(e) {
        resolve(null);
      });
      audio.src = window.filePath(file);
    });
    return s;
  });
